...
public class PartidoService {
    ...
    public PartidoService(IPartidoRepository partidoRepository, BuscadorPartidos buscadorPartidos,
    ...
    public List<Partido> buscarPartidos(Jugador jugador) {
        return buscadorPartidos.buscarPartidos(partidoRepository.findAll(), jugador);
    }
    ...
    public void actualizarEstadosPorTiempo() {
        // reuse previous logic: handle NecesitamosJugadores -> EnJuego etc.
        LocalDateTime ahora = LocalDateTime.now();
        for (Partido p : partidoRepository.findAll()) {
            if (p.getHorario() == null) continue;

            if (p.getEstado() instanceof NecesitamosJugadores && !ahora.isBefore(p.getHorario())) {
            ...
            if (p.getEstado() instanceof PartidoArmado && !p.todosConfirmados()) {
                LocalDateTime unaHoraAntes = p.getHorario().minusHours(1);
                if (!ahora.isBefore(unaHoraAntes)) {
            ...
            } else if (p.getEstado() instanceof Confirmado && ahora.isEqual(p.getHorario())) {
                p.setEstado(new EnJuego());
                notificacionController.notificarTodos(p,
                        "El partido de " + p.getDeporte() + " en " + p.getUbicacion() + " ha comenzado!");
            } else if (p.getEstado() instanceof EnJuego) {
                LocalDateTime fin = p.getHorario().plusMinutes(p.getDuracion());
